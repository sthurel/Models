...
def evaluate_calendar_context(current_date: datetime) -> tuple[str, str, str]:
    """
    Evaluates context:
    ...
    # 1. Check for upcoming events in the 30-45 day window
    for event_date, event_name in calendar_events:
        ...
        if 30 <= days_until <= 45:
            ...
            if (45 - days_until) < burst_duration:
                ...
                return "Seasonal / Calendar Event", subject, context
    ...
    if current_date.month in [12, 1, 2, 3]:
        ...
        return "Pre-Summer Niche Calendar", subject, context
    ...
    return "Pop Culture / Web Trends", subject, context
...
def generate_pod_model(
    ...
    trigger_type, subject, context_desc = evaluate_calendar_context(ref_date)
...
def export_daily_pod_model(
    filepath: str = "pod_daily_models.json",
    count: int = 1,
    append: bool = True,
    target_date: datetime | None = None,
) -> None:
    """Exports generated daily model records to a JSON file."""
    existing_data = []
    ...
    new_entries = [
        generate_pod_model(
            design_id=len(existing_data) + i + 1, target_date=target_date
        )
    ...
    print(f"Successfully generated {len(new_entries)} design model(s) to '{filepath}'.")
